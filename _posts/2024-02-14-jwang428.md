...
For KRT80, it has been noted to be upregulated in endocrine resistant ERα breast cancer [9]. 
...
clusters_PCA <- ggplot(data = df_cluster, aes(x = PC1, y = PC2, col = cell_type)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  guides(color = guide_legend(override.aes = list(size = 2))) +
  labs(title = 'Clusters of All Cells in PCA Space', col = "Cell Type")
...
grid.arrange(clusters_2D, clusters_PCA,
             ...
             volcano_plot,
             layout_matrix = lay,
             top = "Analyzing the Cell Identity of Cluster 9")
