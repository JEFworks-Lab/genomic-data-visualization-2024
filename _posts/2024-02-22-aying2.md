...
?Rtsne
set.seed(39)
emb <- Rtsne(pexpnorm, perplexity=20)$Y
...
p2 <- ggplot(df3) + 
    ...
    scale_color_manual(values = c("gray","darkorange", "darkorchid"), 
                       labels = c("1, 3, 4, 5, 7", "2", "6"))+
    labs(col='cluster') +
    labs(title = 'Cluster vs. aligned_y vs. aligned_x (k = 7) (Physical Space)') +
...
p3 <- ggplot(df4) + geom_point(aes(x = logfc, y = logpv, color=diffexp)) +
    ...
    geom_text_repel(aes(label=ifelse(diffexp != "Not Significant",as.character(gene), "")
...
p8 <- ggplot(df6) + 
    geom_point(aes(x = x, y = y, col=Podoplanin), s
    ize=0.8) +
